...
% + toc=fancy               Fancier table of contents with support for subsections
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item[\texttt{white}] Title page with no picture\footnote{This is the default option if no (other) option is specified}
...
\begin{frame}
    \frametitle{Table of Contents (TOCs)}
...
\begin{frame}
    ...
    \end{itemize}
    \footnotetext<3>{See this frame for reference; only when all contents are displayed, the frame counter turns orange}
\end{frame}
...
% starts a new TeX group to make changes to logo color and background image only locally
{
    ...
    \setbeamertemplate{background}{%
        \begin{tikzpicture}
            \node at (current page.center) [outer sep=0pt, inner sep=0pt] {\includegraphics[height=\paperheight]{images/weiden-orig.png}};
        \end{tikzpicture}%
    }
    \setbeamercolor{footline}{fg=weiss, bg=grau2}
    \begin{frame}{\color{weiss}Frame title can be white, too}
        \begin{block}{}
...
\begin{frame}
    ...
    \begin{center}
        ...
        the title not displayed but it affects layout\footnote[3]{The next slide has a title, but notice how the layout is not affected}.\\[1cm]
...
\begin{frame}
    ...
    \begin{center}
        However, if you specify \texttt{\textbackslash{}frametitle\{\{\}\}},\\
        the title not displayed but it affects layout\footnote[3]{\hspace*{\widthof{The next}-\widthof{This}}This slide has a title, but notice how the layout is not affected}.\\[1cm]
        Use this when your diagram obstructs the OTH logo.
    \end{center}
\end{frame}
